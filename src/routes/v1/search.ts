...
function buildSqlFilter(alias: string, body: Pick<FtsSearchRequest, "source" | "kind" | "project" | "session" | "visibility">): { sql: string; params: unknown[] } {
  const where: string[] = [];
  ...
  if (body.visibility) { where.push(`json_extract_string(${alias}.extra, '$.visibility') = ?`); params.push(body.visibility); }
