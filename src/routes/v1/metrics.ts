...
const rawMongoGaugeRefreshMs = Number(process.env.OPENPLANNER_METRICS_MONGO_REFRESH_MS ?? 60_000);
const mongoGaugeRefreshMs = Math.max(
  5_000,
  Math.min(10 * 60_000, Number.isFinite(rawMongoGaugeRefreshMs) ? rawMongoGaugeRefreshMs : 60_000),
);
