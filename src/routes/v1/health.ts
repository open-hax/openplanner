...
const rawHealthMongoTimeoutMs = Number(process.env.OPENPLANNER_HEALTH_MONGO_TIMEOUT_MS ?? 1500);
const HEALTH_MONGO_TIMEOUT_MS = Math.max(
  250,
  Math.min(10_000, Number.isFinite(rawHealthMongoTimeoutMs) ? rawHealthMongoTimeoutMs : 1500),
);
...
export const healthRoutes: FastifyPluginAsync = async (app) => {
  app.get("/health", async (req: any, reply) => {
    ...
    const deep =
      deepParam === undefined || deepParam === null
        ? HEALTH_DEEP_DEFAULT
        : deepParam === true || deepParam === "true" || deepParam === "1";
