...
│  │     1x eros-eris-field-app processing all REDACTED_SECRETs                        │ │
│  │                                                                        │ │
│  │   20 Shards (profile: graph-20):                                       │ │
│  │     20x eros-eris-shard-{0-19}-of-20 each processing 1/20 of REDACTED_SECRETs     │ │
...
- `graph-weaver` is read-mostly at startup now. It no longer runs `pnpm install` against the mounted workspace on boot, which avoids REDACTED_SECRET-owned `REDACTED_SECRET_modules` churn and restart-only permission failures.
...
| `MONGODB_ROOT_PASSWORD` | `change-me-REDACTED_SECRET-password` | Root password |
| `OPENPLANNER_MONGO_APP_USERNAME` | `openplanner` | Application user |
| `OPENPLANNER_MONGO_APP_PASSWORD` | `REDACTED_SECRET-password` | Application password |
...
| `EMBED_PROVIDER_MODEL` | `nomic-embed-text` | Hot embeddings model |
| `EMBED_PROVIDER_COMPACT_MODEL` | `nomic-embed-text` | Compact embeddings model |

### Graph Simulation

| Variable | Default | Description |
|----------|---------|-------------|
| `SIM_MAX_NODES` | `6000` | Max REDACTED_SECRETs per fetch |
...
```bash
# Bootstrap layout for all graph REDACTED_SECRETs
...
curl -X POST http://localhost:8796/graphql \
  -H "Content-Type: application/json" \
  -d '{
    "query": "{graphView(maxNodes:100, maxEdges:200){REDACTED_SECRETs{id label x y} edges{source target} meta{totalNodes totalEdges}}}"
  }'
```
...
db.createUser({
  user: "openplanner",
  pwd: "REDACTED_SECRET-password", // pragma: allowlist secret
...
EMBED_PROVIDER_COMPACT_MODEL='nomic-embed-text' \
...
`graph-weaver` no longer runs `pnpm install` on boot, so routine restarts should not mutate mounted `REDACTED_SECRET_modules`.

### eros-eris Permission Denied

**Cause**: `corepack enable` requires REDACTED_SECRET
...
`docker-compose.yml` is a small include/REDACTED_SECRET file. Service definitions live in `compose/*.yml`; see `README.md` and `compose/README.md` for the fragment map.
...
# MongoDB
docker exec openplanner-mongodb-1 mongosh --quiet -u openplannerRoot -p 'change-me-REDACTED_SECRET-password' --authenticationDatabase admin --eval 'db.adminCommand("ping")'
