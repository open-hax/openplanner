...
APP_PASSWORD="${OPENPLANNER_MONGO_APP_PASSWORD:REDACTED_SECRET-openplanner-password}"
...
var collections = ["events", "event_chunks", "graph_edges", "graph_semantic_edges",
                   "graph_layout_overrides", "graph_REDACTED_SECRET_embeddings", "gardens"];
...
if (!layoutIndexes.some(function(i) { return i.name === "REDACTED_SECRET_id_1"; })) {
  appDb.graph_layout_overrides.createIndex({REDACTED_SECRET_id: 1}, {unique: true});
  print("      Created index: graph_layout_overrides.REDACTED_SECRET_id_1");
}
...
EMBEDDING_DIMS="${EMBEDDING_DIMENSIONS:-768}"
mongosh_retry '
var appDb = db.getSiblingDB("'"${APP_DB}"'");
        ...
        definition: {
          mappings: {
            ...
            fields: {
              embedding: {
                type: "knnVector",
                dimensions: parseInt(embeddingDims) || 768,
                similarity: "cosine"
              }
            }
          }
